...
public class MySQLiteOpenHelper4 extends SQLiteOpenHelper {
    ...
    // 앱설치 후 최초에 데이터 베이스가 '없는 경우', 데이터 베이스 생성을 위해 호출되는 콜백
    ...
    // 데이터베이스의 '버변' 이 바뀌었을 때 호출되는 콜백 메소드
    // '버전' 이 바뀌었을 때 기존에 설치 운영되고 있는 데이터 베이스로 어떻게 변경할 것인지 작성
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        String sql = "DROP TABLE IF EXISTS mytable";  // 기존 테이블 삭제    IF EXISTS는 오라클에서는 못쓰고 SQL문에서만 쓰임
        db.execSQL(sql);
